...
abstract: |
  Regression is powerful — and easily misused. This chapter introduces ordinary least 
  squares regression, coefficient interpretation, hypothesis testing in a multivariate 
  context, and the key assumptions that must hold for results to be trustworthy. Working 
  through Sunburst Homes Construction's pricing analysis, where a statistically 
  significant coefficient masks confounding, multicollinearity, and heteroskedasticity, 
  students learn not just how to run a regression but how to interrogate one — and when 
  not to act on it.
---
